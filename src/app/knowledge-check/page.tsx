...
const TABS: { id: TabId; label: string; subtitle: string; quizKey: string }[] = [
...
export default function KnowledgeCheckPage() {
  ...
  // Load and refresh scores whenever the active tab changes
  useEffect(() => {
  ...
  const attemptedCount = TABS.filter(t => scores[t.quizKey] !== undefined).length;
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Three interactive quizzes to verify what you have learned about Databricks and
      ...
      <div className="grid grid-cols-3 gap-3">
        {TABS.map(tab => {
          ...
          return (
            ...
            >
              <p className="text-xs font-semibold text-gray-600">{tab.label}</p>
              ...
              {done ? (
                ...
                <p className="text-xs text-gray-400 mt-1.5 italic">Not attempted</p>
              )}
            </div>
          );
        })}
        ...
        <div className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-gray-50 border border-gray-100">
          <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${Math.round((attemptedCount / TABS.length) * 100)}%` }}
            />
          </div>
          <span className="text-xs text-gray-500 font-medium whitespace-nowrap">
            {attemptedCount}/{TABS.length} quizzes completed
          </span>
        </div>
      )}
